...
public interface IDependencyDataAccess
{
    ...
    /// </summary>
    DependencyGraphStats GetStats(string projectPath);

    /// <summary>
    /// Gets direct dependencies for an asset.
    /// </summary>
    DependencyEdge[] GetDependencies(string projectPath, string assetPath, int limit);

    /// <summary>
    /// Gets direct dependents for an asset.
    /// </summary>
    DependencyEdge[] GetDependents(string projectPath, string assetPath, int limit);

    /// <summary>
    /// Gets a bounded related cluster from an asset.
    /// </summary>
    string[] GetRelated(string projectPath, string assetPath, int maxDepth, int limit);

    /// <summary>
    /// Searches assets by class name.
    /// </summary>
    ClassSearchHit[] SearchByClass(string projectPath, string className, int limit);

    /// <summary>
    /// Searches property values by property name and optional value filter.
    /// </summary>
    PropertySearchHit[] SearchProperties(string projectPath, string propertyName, string? valueFilter, int limit);

    /// <summary>
    /// Gets a bounded metadata snapshot for a single asset.
    /// </summary>
    AssetMetadataSnapshot? GetAssetMetadata(string projectPath, string assetPath, int rowLimit);
}
