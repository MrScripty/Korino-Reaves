...
public interface IDependencyGraphCapability
{
    ...
    /// </summary>
    DependencyEdge[] GetDependencies(string assetPath, int limit = 100);

    /// <summary>
    /// Gets direct dependents for an asset path.
    /// </summary>
    DependencyEdge[] GetDependents(string assetPath, int limit = 100);

    /// <summary>
    /// Gets a related cluster around an asset path.
    /// </summary>
    string[] GetRelated(string assetPath, int maxDepth = 3, int limit = 200);

    /// <summary>
    /// Searches assets by class name.
    /// </summary>
    ClassSearchHit[] SearchByClass(string className, int limit = 100);

    /// <summary>
    /// Searches properties by name and optional value filter.
    /// </summary>
    PropertySearchHit[] SearchProperties(string propertyName, string? valueFilter = null, int limit = 100);
}
